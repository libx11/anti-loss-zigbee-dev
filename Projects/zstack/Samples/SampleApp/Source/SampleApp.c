...
void beep0(void)
{
    T1CC0H = 0x00;           //0ms的周期时钟
    T1CC0L = 0x00;
}
void beep1(void)
{
    T1CC0H = 0x00;           //1ms的周期时钟
    T1CC0L = 0xff;

}
void beep2(void)
{
    T1CC0H = 0x01;           //2ms的周期时钟
    T1CC0L = 0xff;
}
...
#if defined(ZDO_COORDINATOR)                      //协调器短地址为0000
    {
        device[0] = 0x0000;
    }
#endif
...
void SampleApp_HandleKeys( uint8 shift, uint8 keys )
{
    ...
    else if ( keys & HAL_KEY_SW_6 )//key1
    {
      mode_change();
    }
}
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
    ...
    case SAMPLEAPP_PERIODIC_CLUSTERID: //收到广播数据
        ...
        if(buf[0]=='D' && buf[1]=='1')      //广播数据
        {
            ...
            if(main_flag == 0)                	//model 0 停止
            {
                beep0();

            }
            else if(main_flag == 1)         //mode 1 标准模式 根据距离智能判断是否丢失
            {
    ...
    case SAMPLEAPP_SINGLE_CLUSTERID:        //点对点呼叫数据
        #if defined ( LCD_SUPPORTED )
...
void SampleApp_SendBeepMessage(uint16 dst, uint8 s)
{
    ...
    if( AF_DataRequest( &SampleApp_Single_DstAddr,   //发送目的地址＋端点地址和传送模式
                        &SampleApp_epDesc,            //源(答复或确认)终端的描述（比如操作系统中任务ID等）源EP
                        SAMPLEAPP_SINGLE_CLUSTERID, //被Profile指定的有效的集群号
                        3,                // 发送数据长度
                        SendData,                          // 发送数据缓冲区
